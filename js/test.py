...
Anzahl_Sat = ""
# UART oeffnen
UART = serial.Serial("/dev/ttyACM0", 4800) 
#UART.open()
# Startmeldung
while True:
	Protokolle = {}
    ...
    #token = open('/var/www/html/token.txt', 'r').read();

	#print "http://www.menue-catering.de/api/temp_mon/write.php?token="+str(token)+"&koordinate_breite="+str(float(Koordinaten_Breite))+"&koordinate_laenge="+str(float(Koordinaten_Laenge));
	#contents =urllib2.urlopen("http://www.menue-catering.de/api/temp_mon/write.php?token="+str(token)+"&koordinate_breite="+str(Koordinaten_Breite)+"&koordinate_laenge="+str(Koordinaten_Laenge)).read()
	#print contents

    contentString = "&koordinate_breite="+str(Koordinaten_Breite)+"&koordinate_laenge="+str(Koordinaten_Laenge)

    gpsFile.write(contentString)
    gpsFile.close()
